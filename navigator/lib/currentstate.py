"""AA11393US claims navigator — current-state derivation and certification.

Read-only current-state plane (TDD §10, §11): shared candidate
derivation, pin plans, approval and QA authorization predicates, bundle
proposal state, the verify-current closure, and the status report.
Command adapters in build.py own argument parsing and boundary
construction and call into this module; nothing here writes to the
content, record, or artifact planes.
"""
...
from . import acceptance, authority, bundleplan, bundlezip, canon
from . import claims as claims_mod, control_inventory, gateway, inputlock
from . import model, pinplan, profilepolicy, qaevidence, qaregistry
from . import recordprovenance, recordresolver, registry as registry_mod
from . import render, schema_validate, snapshot, validate
...
def edition_qa_registry(m):
    """Load the edition's QA-only registry outside the content lock.
...
def attestation_evidence_problems(attestation, side_digests, edition_id=None):
    ...
    if not isinstance(raw_record, dict) or \
            set(raw_record) != recordprovenance.ATTESTATION_RECORD_FIELDS:
...
def qa_authorization_problems(qa, candidate_digest, content_lock,
    ...
    if not isinstance(raw_record, dict) or \
            set(raw_record) != recordprovenance.QA_RECORD_FIELDS:
...
def _current_bundle_plan_state(
        command, byte_source=None, load_planes=None):
...
def _verify_current_closure(byte_source, load_planes=None):
    ...
    state = _current_bundle_plan_state(
        "verify-current", byte_source, load_planes)
    cfg = state["config"]
