"""Deterministic GFM review projection and bidirectional coverage."""
...
class _Renderer:
    ...
    def _metadata(self, root: ET.Element, subject_id: str) -> None:
        ...
        if authored is not None:
            self.emit("| Origin | Authored source |",
                      "| Responsible owner | %s |" % _escape_text(authored.get("responsibleOwner")),
                      "| Review scope | %s |" % _escape_text(authored.get("reviewScope")))
        elif pdf is not None:
            stored = pdf.find(C + "storedSource")
            self.emit("| Origin | PDF-derived structured source |",
                      "| Stored source | %s |" % _escape_text(stored.get("path")),
                      "| Stored-source digest | `%s` |" % stored.get("rawDigest"),
                      "| Stored-source size | %s |" % stored.get("size"),
                      "| Stored-source role | %s |" % _escape_text(stored.get("role")),
                      "| Official-copy status | %s |" % _escape_text(pdf.get("officialCopyStatus")),
                      "| Extraction method | %s |" % _escape_text(pdf.get("extractionMethod")))
            for convenience in pdf.findall(C + "convenienceDerivative"):
                self.emit("| Non-authoritative convenience derivative | %s — `%s` |" % (
                    _escape_text(convenience.get("path")), convenience.get("rawDigest")))
        self.emit("")
    ...
    def render_content(self) -> bytes:
        ...
        notice = (
            ...
            "--no-sync python -m structured_source render %s`; edit the XML source, "
            "never this Markdown. -->" %
...
def render_content(artifact: ParsedArtifact, output_path: str,
    ...
    coverage = canonical_json(coverage_value)
    return Projection(
        markdown=markdown,
        coverage=coverage,
        markdown_digest=raw_digest(markdown),
        coverage_digest=raw_digest(coverage),
    )
...
def render_relations(artifact: ParsedArtifact, output_path: str,
    ...
    renderer.emit(
        ...
        "source profile %s; projection profile %s/%s; regenerate with "
        "`uv --no-cache --offline run --locked "
        "--no-sync python -m structured_source render %s`; edit the relation XML, "
        "never this Markdown. -->" %
    ...
    context = root.find(R + "reviewContext")
    if context is not None:
        renderer.emit(_anchor_element("relation-context"),
                      "# Relation-set review context", "")
        for node in context:
            start = len(renderer.lines)
            renderer.lines.extend(renderer.block(node))
            renderer.emit("")
            for identified in node.iter():
                identifier = identified.get(XML_ID)
                if identifier:
                    renderer.mark(identifier, start)

    relation_regions = {}
    renderer.emit(_anchor_element("relation-schedule"),
                  "# Exact relation review schedule", "")
    for relation in root.findall(R + "relation"):
        ...
        renderer.emit(renderer.anchor_line(xml_id),
                      ...
                      "| Review owner | %s |" % _escape_text(relation.get("reviewOwner")),
                      "", "### Exact endpoints", "",
    ...
    def census(node, plane, nearest):
        ...
        if plane == "context" and identifier:
            classification = "review-visible"
            anchors = [_anchor(identifier)]
            region = renderer.regions.get(identifier)
        elif plane == "relation" and identifier:
            classification = "review-scheduled"
            anchors = [_anchor(identifier)]
        ...
        for child in node:
            ...
            child_plane = plane
            if node is root:
                child_plane = {"identity": "identity", "reviewContext": "context",
    ...
    coverage = canonical_json(coverage_value)
    return Projection(markdown=markdown, coverage=coverage,
                      markdown_digest=raw_digest(markdown),
                      coverage_digest=raw_digest(coverage))
