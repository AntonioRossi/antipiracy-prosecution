...
def load_projection_profile():
    ...
    fields = {"profileVersion", "profileId", "lineEndings", "finalNewline",
              ...
              "generatedNoticeVersion", "externalSchemes"}
    if not isinstance(value, dict) or set(value) != fields or \
            ...
            not isinstance(value.get("stableAnchorPrefix"), str) or \
            _ID.fullmatch(value["stableAnchorPrefix"] + "anchor") is None or \
            not isinstance(value.get("generatedNoticeVersion"), str) or \
            _ID.fullmatch(value["generatedNoticeVersion"]) is None:
        raise StructuredSourceError("GFM projection profile shape/version is not current")
    schemes = value.get("externalSchemes")
    if not isinstance(schemes, list) or len(schemes) != len(set(schemes)) or \
            not schemes or not all(isinstance(item, str) and item for item in schemes):
        raise StructuredSourceError("GFM external-scheme registry is not exact")
    return value
