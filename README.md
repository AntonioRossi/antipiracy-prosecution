...
| [`navigator/`](navigator/RUNBOOK-content-sync-and-regeneration.md) | Edition-blind HTML5 navigator source, closed schemas, reviewed NA/AF mappings, exact active verification-record graph, tests, runbook, and committed current artifacts |
...
| [`AA11393US-structured-source-markdown_technical-description.md`](AA11393US-structured-source-markdown_technical-description.md) | Normative structured-source contract, coupled to its acceptance criteria |
...
One command proves the complete structured-source and navigator closure inside immutable repository snapshots: declared authority direction, XML identities, deterministic representations, computed coverage, reference and consumer closure, pin plans, candidate and sealed bytes, bundle and authorization chains, exact navigator record and distribution inventories, and `git diff --check` whitespace. It runs both registered test families in a materialized sandbox, renders every current changed Markdown file through pandoc, verifies all co-located `US/prior-art` source manifests, and certifies only the final unchanged snapshot. The audit unit is the exact clean Git commit, its checkout and supplied history, the structured-source documentation pair and executable registries, and this current gate result. No export, stored receipt, reviewer record, or self-attestation substitutes for that execution. The runbook's [current-state and cutover gate](navigator/RUNBOOK-content-sync-and-regeneration.md#7-current-state-and-cutover-gate) section places the gate in the full content-integration workflow.

## Status discipline

Package documents, live configuration, structured-source registries, navigator verification stores, and current build products contain only the current operative version, status, conclusion, supporting evidence, action, owner, deadline, trigger, schema, content binding, and records required by the active navigator authorization graph. Git is the sole drafting, implementation, and displaced-record history; do not retain revision records, dated maintenance histories, wording-evolution narratives, non-current records or artifacts, compatibility aliases, or implicit upgrades in the live checkout. Structured source has no approval inventory or record, reviewer registry, self-attestation, implementation register, stored coverage artifact, or export bundle. Computed coverage remains mandatory. The navigator store, when required by its contract, contains exactly its active reachable records. Retain legally operative filing, publication, priority, and prosecution facts and source provenance as current evidence. Every strategy document states its strategy ID, version, status, and review date.
