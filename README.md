...
| Location | Function |
|---|---|
| [`US/`](US/) | US claiming strategies: `normal-allowance/` (NA baseline), `allowance-first/` (AF alternative plus AF-CONT continuation candidate), `common/` shared controls, `prior-art/` canonical art store |
...
| Memo | `…_MEMO.md` | Controlling instrument (continuation preservation, deferred filing/disclosure/EP work) |
| Claim crosswalk | `…-claim-crosswalk_DRAFT.md` | Inter-strategy claim mapping (AF parent to AF-CONT) |
| Shared record | `US/common/…_DRAFT.md` / `…_MEMO.md` | Single IDS inventory and single PCT informal-comments draft; referenced, never duplicated |
...
3. **Route the strategy.** [`US/README.md`](US/README.md) defines the strategy IDs (NA, AF, AF-CONT), current claim-set versions, and status controls. Use `NA claim N`, `AF claim N`, and `AF-CONT claim N` outside claim text; an unqualified claim number must not transfer a conclusion between strategies.
4. **Draft the claims.** Actor-focused independent claims, singly dependent fallbacks, no multiple-dependent claims, within current numerical ceilings, applying the exclusions recorded in each claim set (no suspect-side physical-camera identification, no joint ordered-source-pair-plus-timing matching, without a new claim-as-a-whole support determination).
5. **Produce the companion artifacts.** Each claim set is valid only with its versioned companions: priority-support map (separate PCT and provisional written-description/enablement conclusions, Mode A/B/C gating), prior-art comparison matrix, claim-document mapping matrix, and counsel briefing; AF-CONT is additionally controlled by the AF claim crosswalk and the continuation-preservation memo. A matrix is valid only for the claim-set version named in its header.
6. **Apply the shared controls.** [`US/common/`](US/common/) holds the single IDS inventory, the single PCT informal-comments draft, and the deferred filing/disclosure/formalities/EP work memo. Coordinate arguments, priority positions, and IDS decisions across strategies to avoid contradictory records.
