...
| **"Equal" manifest** | The lookup key matching a delivered manifest to a reconstructed one; counsel must select one supported construction (byte identity, equivalent chunk selections, or equivalent represented timing choices) and account for dynamic URLs, tokens, and metadata | AF-CONT claim set § 3, reconstruction and equality gate |
