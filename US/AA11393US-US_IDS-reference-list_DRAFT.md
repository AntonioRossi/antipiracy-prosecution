...
A further claim-informed supplemental review was performed on 16 July 2026 in view of the revised candidate claim set (four-actor architecture; camera-cut timing patterns; manifest interleaving; camera-source-transition detection). It comprised citation mining of the A4 grant face and of the face of D1's US grant (US 11,540,029 B2), plus targeted keyword sweeps for the comparison-matrix §5 gap areas. References added from that review are marked accordingly below. This too was a limited review and is not represented as exhaustive.
...
| A6 | US 2014/0325550 A1 | 2014-10-30 | Verance Corporation; Winograd et al. | *Real-Time Anti-Piracy for Broadcast Streams* | Examiner-cited on A4's face. Delivery of content segments where a requested segment differs from another in at least one characteristic; real-time response to piracy of broadcast streams; relevant to distribution claims 9–15 and detection-response context | Supplemental candidate identified 16.07.2026 from the A4 face-citation review; copy stored; counsel materiality review required |
...
| B9 | WO 2009/156973 A1 | 2009-12-30 | France Telecom SA (inventors Liu, Lian, Wang, Ren) | *Fingerprinting Method and System*; per-user version assembled by sequentially selecting consecutive segments from space-desynchronized streams at predefined exchangeable time points, with watermarks embedded in the selected segments; cited on the face of D1's US grant. Full text 16.07.2026: detection is not elaborated in this publication (embedding side only) | Supplemental candidate identified 16.07.2026 from the D1-grant face-citation review; copy stored. **The B2 ISR rates B9 itself category X against D1's PCT claims 1–10, 13–17, 19–21, 24–30, pinpointing i.a. the exchangeable-time-points passages.** Sibling WO 2009/122385 A2 (fingerprint encoding by watermark elimination) reviewed and provisionally set aside as cumulative of A2/C3 concepts — counsel to confirm both decisions |
...
| C8 | ETSI TS 104 002 V1.1.1 (2023-08), “Publicly Available Specification (PAS); DASH-IF Forensic A/B Watermarking” | Standardized architecture in which at least two watermarked variants (A/B) of ABR segments are delivered and per-session variant sequences identify recipients; published August 2023, before the 26.02.2024 provisional. Official ETSI copy stored 16.07.2026; relevant §103 context for distribution claims 9–15 |
...
| A6 / A7 / A8 / C8 | Per-session delivery of differing or watermarked segment variants selected via manifests or requested addresses; device/recipient association; real-time piracy response; obfuscation of the variant pattern including randomized manifest-entry durations; a standardized A/B watermarking architecture (ETSI, 08.2023) | Variants created at the multi-camera production stage by moving a recorded camera-selection boundary so an interval contains temporally corresponding frames from another camera, with detection recovering camera-source transitions rather than embedded marks |
...
| B9 | Per-user versions assembled by sequentially selecting consecutive segments from space-desynchronized streams; embedded fingerprint watermarks; fingerprint detection | Segment variation sourced from synchronized alternate-camera selection at recorded cut boundaries, with manifest/ledger association and structural recovery |
...
- [ ] Review the 16.07.2026 supplemental references A6–A10, B9, C7, C8 for materiality against the final claim set and record submit/skip decisions; decide whether D1's grant US 11,540,029 B2 is listed in addition to A1. A10 was promoted from the B2-ISR X citations after initially being set aside; the same ISR corroborates B6-family, B9, and C7 materiality (X against D1's own PCT claims). **Full-text re-score of the claim–document grid completed 16.07.2026 — note the A4 camera-perspective finding flagged per memo §7.**
