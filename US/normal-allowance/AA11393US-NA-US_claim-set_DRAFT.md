...
| 19–20, 25 | Direct support for deriving a time code, building reconstructed manifest 121′, and searching the ledger: PCT claim 15 and the detection-component 130 and retrieval-component 140 passages. Context for the underlying manifest structure: PCT Example 3 and the provisional's unnumbered manifest-files passage at PDF pp. 28–29. Those contextual passages do not independently supply the claimed derive/build/search operations. |
...
2. **Broader production theory.** NA claim 1 deliberately requires the observable same ordered first-camera-to-second-camera transition at noncoincident timings. Preserve the former broader production theory—local alternate-camera substitution without the same-pair requirement—only in a coordinated continuation or other counsel-approved claim strategy; do not argue that the present claim 1 omits the same-pair limitation. A generic relative-timing clause alone may be vulnerable to D1's frame-rate transformation.
...
9. **CRM alternative.** If a medium claim is desired, draft it for the software operations of NA claim 1 or NA claim 16 rather than the entire production/distribution/detection chain.
...
12. **Positive collusion output.** NA claims 21 and 28 affirmatively require suspected content assembled from different delivered versions and identification of one or more contributing recipients. NA claim 21 additionally recites attribution scores. Counsel should preserve positive performance of the attribution operation, confirm support for the scoring formulation, and avoid optional “when” language in a method claim.
13. **Detection-dependent terminology.** NA claims 17 and 18 expressly address detection of the ordered camera-source transition and corresponding camera-switch timing to remain aligned with NA claim 16. NA claim 19 intentionally remains timing-specific because it derives a time code and reconstructs manifest information from that time code; do not broaden it merely for terminological symmetry.
14. **A4/B9 construction discipline.** NA claims 1, 9, and 16 recite the same ordered transition at noncoincident positions and the alternate-camera interval between them because generic camera-perspective fragments, per-user manifest sequences, and variable stream-switch positions are known. Counsel should resist deleting those structural limitations unless a materially broader position is supported by a completed search and a reasoned validity analysis.
15. **NA claim 9 proof trade.** The structural limitations improve the patentability and validity position but increase the infringement-proof burden. Before finalizing NA claim 9, counsel should plan controlled captures from multiple recipient sessions and assess discovery access to manifest/source-chunk mappings and recipient-association records; actor splitting should not be treated as a pure enforcement gain.
16. **NA claim 1 proof ladder.** The same ordered transition and noncoincident timings in NA claim 1 are output-level relationships that may be investigated through synchronized reference/mate comparison and source-feed evidence. The structured-list input and automatic time-code variation will ordinarily require internal records or discovery; NA claim 7 intentionally adds the narrower proof that the varied structured list retained the corresponding source-camera identifiers.

## 7. Revision record

- **NA-2026-07-21-v2 (21 July 2026):** moved the observable same ordered first-camera-to-second-camera transition at noncoincident reference/mate timings into NA claim 1; retained source-camera-identifier preservation and recorded-time-code change as the narrower NA claim 7 structured-list fallback; left the text and added resynchronization limitations of claims 2–3 unchanged while those claims inherit amended claim 1; and reserved the former broader production theory for coordinated continuation review.
