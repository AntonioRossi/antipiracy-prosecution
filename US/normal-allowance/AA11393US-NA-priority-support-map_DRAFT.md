...
| Same ordered camera-source transition occurs at noncoincident time codes while retaining the source identifiers on its respective sides (NA claims 7, 9, 15, 16, 22, 29) | pp. 26–27: the reference and mate EDL tables retain the Camera 2-to-Camera 3 order while Cut 2 is extended and Cut 3 begins ten frames later in the mate | Example 2, Tables 1–3 and explanatory text | Supported by the concrete example; counsel must confirm that generalizing the shown ordered transition and two timings is an adequate written-description expression rather than an unsupported abstraction |
...
|---|---|---|---|
| NA claims 1–6 — production | pp. 15, 20–27, 42–44 | PCT claims 1, 10, and 16; system 100; PCT Examples 1–2 | High for core other than the specific resynchronization fallback in NA claims 2–3, which requires the Example 2 inconsistency analysis above; proposed wording to verify |
| NA claim 7 — retained ordered transition / changed recorded time code | pp. 26–27, 42–44 | PCT Example 2, Tables 1–3; PCT claims 10 and 16 | Concrete example retains the Camera 2-to-Camera 3 ordering while changing the boundary time; counsel must confirm support and priority for the generalized first-camera/second-camera formulation |
...
8. Confirm whether NA claim 7 support-safely generalizes Example 2 as retaining the same ordered source identifiers while changing the recorded transition time.
