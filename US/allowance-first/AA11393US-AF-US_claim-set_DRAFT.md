...
This branch deliberately places the disclosed production-to-attribution loop in **one independent system claim**. Its allowance-first premise is that the strongest present distinction is not any isolated use of segmentation, manifests, timing fingerprints, shot detection, or a recipient ledger. It is the operational combination in which:

1. the same ordered transition between identified physical camera sources is deliberately placed at different times in generated versions;
...
AF claim 1 therefore integrates the substance of NA claims 1, 7, 16, and 22. Each output is used by a later operation: the varied edit instructions determine generated content; generated content determines delivered versions and candidate patterns; those patterns and deliveries populate the recipient record; and a jointly source-and-time-matched suspect pattern selects the record entry used for recipient identification. The independent claim is intentionally not a list of unrelated production, distribution, and detection modules.
...
**Count:** 19 total claims; 1 independent claim; 18 singly dependent claims; no multiple-dependent claims.
...
generate a mate of the reference audio-video content by varying, in the structured list of edit instructions, a recorded time code of a selected camera cut at which selection changes from a first camera identified by a first source-camera identifier to a second camera identified by a second source-camera identifier, while retaining the same ordered transition from the first camera to the second camera, such that:
...
|---|---|---|
| Integrated structural core | 1 | Requires the full closed loop and joint source-pair/timing match at the same distinguishing region |
...
1. **AF claim 1 claim-as-a-whole gate.** The PCT and provisional disclose the overall generation, manifest/recipient association, detection, and lookup architecture. The exact same-region relationship and joint matching of both source order and timing are an abstraction formed by reading the Example 2 production disclosure with the detection and retrieval disclosure. Counsel must provide a claim-as-a-whole § 112(a) and priority analysis; isolated support for each noun is not enough.
2. **Source-identity detection gate.** Generic cut-time detection is express. Detecting the identities of both camera sources on the two sides of a suspect transition, and matching that ordered pair together with timing, is not stated with the same specificity in the detection example. AF claims 1 and 7–10 must not be represented as having settled direct support for that operation.
3. **Examples 2–5 integration gate.** AF claims 7–10 and 13–18 combine the moved physical-camera boundary in Example 2 with later manifest, mixing, comparison, or reconstruction passages. Counsel must decide whether the application presents those operations as one integrated system and whether the provisional supports the same combination.
...
5. **AF claim 14 gate.** Equal-duration corresponding reference/mate chunks are express, but the proposition that each paired same-interval chunk itself spans the ordered physical-camera transition at its respective internal timing is not stated verbatim in one passage. Treat AF claim 14 as a combined-example fallback pending counsel review.
...
1. **Single-entity proof.** AF claim 1 spans production, delivery/association, detection, and lookup. A validity-oriented allowance anchor is not necessarily the best infringement claim. Counsel should analyze direction/control, system use, divided infringement, and available evidence before selecting it as the sole independent claim in an application.
2. **Preserve the NA actor split.** Do not abandon the NA production, distribution, and detection families merely because AF claim 1 is narrower. Use continuation, divisional, or parallel-claim planning only as supported and commercially justified.
...
6. **Restriction and unity.** One independent claim may reduce claim-count complexity, but its production, distribution, and detection subject matter can still raise search, unity, restriction, or election issues. Route-specific consequences remain for counsel.

## 7. Track One and formal check

- **Numerical check:** 19 total / 1 independent / no multiple-dependent claim. The set is within the 30-total/4-independent/no-multiple-dependent numerical limits applicable to Track One and within the basic 20-total/3-independent fee allocation.
...
- **Claim-form check:** all dependents refer to one earlier claim; no multiple-dependent claims appear. Convert the draft to the filing route's required original-claim or amendment format and recheck antecedent basis, claim status, and fees.
